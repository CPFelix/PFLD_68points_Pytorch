...
def test_images():
    model_path = './models/model0/model_99.pth'
    images_dir = '/home/chenpengfei/dataset/hand_labels/manual_test/images/'  #'./data/test_data/imgs/'
    image_size = 112  # 112

    # model = BlazeLandMark(nums_class=42)
    model = MobileNetV2(nums_class=42)
...
def test_images1():
    ...
    num_of_channels = [int(64 * coefficient), int(128 * coefficient), int(16 * coefficient), int(32 * coefficient),
                       int(128 * coefficient)]
    model = MobileNetV2(num_of_channels=num_of_channels, nums_class=42)
    model = torch.load(model_path)
    model.eval()
    image_files = os.listdir(images_dir)
    for index, image_file in enumerate(tqdm(image_files)):
        ...
        input = cv2.cvtColor(image_resize, cv2.COLOR_BGR2RGB)
        input = input.astype(np.float32) / 255.0
        input = np.expand_dims(input, 0)
        input = torch.Tensor(input.transpose((0, 3, 1, 2)))
        ...
        pre_landmarks, _ = model(input.cuda())
        pre_landmark = pre_landmarks[0].cpu().detach().numpy()
        pre_landmark = pre_landmark.reshape(-1, 2) * image_size
        for (x, y) in pre_landmark.astype(np.int32):
            cv2.circle(image_resize, (x, y), 2, (0, 0, 255), 2)
        image_draw = cv2.resize(image_resize, (width, height))
        imgName = "./models/model0/test/" + image_file
        cv2.imwrite(imgName, image_draw)
        # cv2.imshow('0', image)
        # if cv2.waitKey(0) == 27:
        #     break

def main():
    model_file = './models/model0/model_99.pth'
    videl_file = './dfl.mov'
    image_size = 112  # 112
